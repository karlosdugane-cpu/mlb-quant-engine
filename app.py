...
def calculate_kelly_criterion(win_prob, decimal_odds, fraction=KELLY_FRACTION):
    if decimal_odds <= 1.0:
        return 0.0
    b = decimal_odds - 1.0
    f_star = (win_prob * b - (1.0 - win_prob)) / b
    return round(f_star * fraction * 100, 2) if f_star > 0 else 0.0
...
def run_mlb_analysis():
    ...
    for game in games:
        ...
        away_kelly = calculate_kelly_criterion(away_prob_dec, odds["away_odds"])
        home_kelly = calculate_kelly_criterion(home_prob_dec, odds["home_odds"])
